...
resource "aws_vpc" "main" {
  ...
  tags = merge(var.tags, {
    Name        = "custom-vpc-${terraform.workspace}"
...
resource "aws_default_security_group" "default" {
  ...
  ingress = []
  egress  = []
  tags    = { Name = "default-sg-locked-${terraform.workspace}" }
}
...
resource "aws_subnet" "public_1" {
  ...
  tags = merge(var.tags, { Name = "public-subnet-1-${terraform.workspace}" })
}
...
resource "aws_subnet" "public_2" {
  ...
  tags = merge(var.tags, { Name = "public-subnet-2-${terraform.workspace}" })
}
...
resource "aws_subnet" "private_1" {
  ...
  tags = merge(var.tags, { Name = "private-subnet-1-${terraform.workspace}" })
}
...
resource "aws_subnet" "private_2" {
  ...
  tags = merge(var.tags, { Name = "private-subnet-2-${terraform.workspace}" })
}

resource "aws_internet_gateway" "gw" {
  vpc_id = aws_vpc.main.id
  tags   = merge(var.tags, { Name = "custom-igw-${terraform.workspace}" })
}
...
resource "aws_route_table" "public" {
  ...
  tags = merge(var.tags, { Name = "public-route-table-${terraform.workspace}" })
}
...
resource "aws_eip" "nat_1" {
  domain = "vpc"
  tags   = merge(var.tags, { Name = "nat-eip-1-${terraform.workspace}" })
}

resource "aws_nat_gateway" "nat_1" {
  allocation_id = aws_eip.nat_1.id
  subnet_id     = aws_subnet.public_1.id
  tags          = merge(var.tags, { Name = "nat-gateway-1-${terraform.workspace}" })
  depends_on    = [aws_internet_gateway.gw]
}

resource "aws_eip" "nat_2" {
  domain = "vpc"
  tags   = merge(var.tags, { Name = "nat-eip-2-${terraform.workspace}" })
}

resource "aws_nat_gateway" "nat_2" {
  allocation_id = aws_eip.nat_2.id
  subnet_id     = aws_subnet.public_2.id
  tags          = merge(var.tags, { Name = "nat-gateway-2-${terraform.workspace}" })
  depends_on    = [aws_internet_gateway.gw]
}

resource "aws_route_table" "private_1" {
  vpc_id = aws_vpc.main.id
  tags   = merge(var.tags, { Name = "private-rt-az1-${terraform.workspace}" })
}
...
resource "aws_route_table" "private_2" {
  vpc_id = aws_vpc.main.id
  tags   = merge(var.tags, { Name = "private-rt-az2-${terraform.workspace}" })
}
